...
import { collection, doc, getDocs, setDoc, addDoc, updateDoc, deleteDoc, query, orderBy, limit, startAfter } from "firebase/firestore";
...
export async function loadMedicalTechnologistNames() {
  return withCache("employees:technologists", async () => {
    const snap = await getDocs(query(collection(db, COLLECTION), orderBy("name")));
    return snap.docs
...
export async function saveEmployee(employeeData) {
  ...
  if (toSave.uid) {
    ...
    cacheInvalidate("employees:technologists");
...
export async function loadEmployeesPage({ cursor = null, pageSize = 20 } = {}) {
