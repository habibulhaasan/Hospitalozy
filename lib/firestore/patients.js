...
export async function searchPatients(searchText) {
  ...
  // To support older patients that don't have searchTokens, we revert to the previous
  // behavior: fetch all patients and filter on the client side. This ensures prefix,
  // partial, and exact matches all work perfectly for both old and new records.
  
  const snap = await getDocs(query(collection(db, COLLECTION), orderBy("name")));
  const all = snap.docs.map((d, idx) => fromDoc(d, idx));
